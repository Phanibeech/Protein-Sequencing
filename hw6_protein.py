...
def runWeek3():
    ...
    elephantProteins = synthesizeProteins("data/chimpanzee_p53.txt", "data/codon_table.json")
...
def runWeek4():
    humanProteins = synthesizeProteins("data/human_p53.txt", "data/codon_table.json")
    chimpanzeeProteins = synthesizeProteins("data/chimpanzee_p53.txt", "data/codon_table.json")

    commonalities = commonProteins(humanProteins, chimpanzeeProteins)
    differences = findAminoAcidDifferences(humanProteins, chimpanzeeProteins, 0.005)
    displayTextResults(commonalities, differences)
...
def runFullProgram2():
    human = synthesizeProteins("data/human_p53.txt","data/codon_table.json")
    chimpanzee = synthesizeProteins("data/chimpanzee_p53.txt", "data/codon_table.json")
    common = commonProteins(human,chimpanzee)
    difference = findAminoAcidDifferences(human,chimpanzee, 0.005)
    displayTextResults(common,difference)
    labels = makeAminoAcidLabels(human,chimpanzee)
    f1= setupChartData(labels,human)
    f2= setupChartData(labels,chimpanzee)
    elist= makeEdgeList(labels,difference)
    createChart (labels,f1,"human",f2,"chimpanzee",edgeList=elist)
    return
